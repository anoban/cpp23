...
__global__ void sum(_In_reads_(NRANDOMS) const int64_t* const numbers, _Inout_count_(NTHREADS) double* const sums) {
...
auto main() -> int {
    ...
    std::vector<int64_t> randoms(NRANDOMS);
    std::generate(randoms.begin(), randoms.end(), rengine);

    const auto host_sum { std::accumulate(randoms.cbegin(), randoms.cend(), 0.0L, std::plus<int64_t> {}) };
    ...
    // TODO: Error handling
    cuStatus = ::cudaMalloc(&d_randoms, memsize<int64_t>);
    cuStatus = ::cudaMalloc(&d_sums, sizeof(double) * NTHREADS);
    cuStatus = ::cudaMalloc(&d_total, sizeof(double));

    cuStatus = ::cudaMemcpy(d_randoms, randoms.data(), memsize<int64_t>, cudaMemcpyHostToDevice);

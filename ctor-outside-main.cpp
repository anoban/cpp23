...
namespace f {
    ...
    [[nodiscard]] static consteval auto func(const T& obj) noexcept -> decltype(obj.operator()()) requires requires { obj.operator()(); } {
        return obj();
    }

    template<typename T> requires std::is_class_v<T>
    [[nodiscard]] static consteval auto fn(const T& obj) noexcept -> typename T::value_type requires requires { obj.operator()(); } {
        return obj();
    }
} // namespace f
